...
namespace SigrokDecoderBridge
{
    public abstract class SigrokDecoderBase : IDisposable
    {
        ...
        public bool ValidateOptions(SigrokOptionValue[] SelectedSettings, SigrokSelectedChannel[] SelectedChannels, CaptureChannel[] Captures)
        {
            ...
            {
                if (!SelectedChannels.Any(c => c.SigrokIndex == reqSig.Index))
                    return false;
            }
        ...
        public SigrokAnnotation[] ExecuteAnalysis(int SamplingRate, SigrokOptionValue[] SelectedSettings, SigrokSelectedChannel[] SelectedChannels, CaptureChannel[] Captures)
        {
            using (Py.GIL())
            {
                ...
                {
                    var channel = SelectedChannels.Where(c => c.SigrokIndex == channels[buc].Index).FirstOrDefault();
                    if (channel != null)
                    {
                        currentState[buc] = Captures[channel.CaptureIndex].Samples[0];
                        lastState[buc] = Captures[channel.CaptureIndex].Samples[0];
                ...
                sampleCount = SelectedChannels == null || 
                    ...
                    0 : Captures[0].Samples.LongLength;
        ...
        {
            //Debug.WriteLine($"Has channel {channel}: {indexedChannels.ContainsKey(channel)}");
        ...
        Dictionary<int, int> CreateCurrentState()
        {
            ...
            {
                state[channelIndex] = captures[indexedChannels[channelIndex].CaptureIndex].Samples[currentSample];
            }
